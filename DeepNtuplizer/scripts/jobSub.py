...
def doSub():
    ...
    globalOutDir=eosGlobalOutDir+'/'+time.strftime('%a_%m%d%H%M%S')
    ...
    for sampledescription in lines:
        ...
        if not isdasname:
            ...
            sample=scriptfile
            #print('sample=scriptfile=',sample)
        ...
        #print('importsample=', importsample)
        cmssource = __import__(importsample)
        #print('cmssource.source=', cmssource.source)
        #print('fileNames=', cmssource.source.fileNames)
        ...
        print('total file number is', totalfiles)
	...
	#logDir=globalOutDir+'/'+outputFile+'/batch/' #'/tmp/'+os.environ['USER']+'/batch/'
        ...
        address.close()
       # ntupleOutDir = '/eos/user/z/zhengz/'
       # print('ntupleOutDir=', ntupleOutDir)
